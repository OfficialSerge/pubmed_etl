...
parser.add_argument(
    "--path", help="path to the directory containing distributed parquet files"
)
...
def insert_table(result, jdbc_url, jdbc_properties, tname):
    result.repartition(10).write.jdbc(
        url=jdbc_url, table=tname, mode="overwrite", properties=jdbc_properties
    )


spark = (
    SparkSession.builder.appName("parquet_unification")
...
george_df = result_df.withColumn(
    "has_abstract", F.when(F.length("abstract") > 0, 1).otherwise(0)
)
george_df = george_df.withColumn(
    "has_title", F.when(F.length("title") > 0, 1).otherwise(0)
)
george_df = george_df.withColumn(
    "has_mesh", F.when(F.length("mesh") > 2, 1).otherwise(0)
)
george_df = george_df.withColumn(
    "has_year", F.when(F.length("year") > 0, 1).otherwise(0)
)

george_df = george_df.select(
    "PMID", "doi", "has_abstract", "has_title", "has_mesh", "has_year"
)
...
jdbc_properties = {"user": user, "password": pas, "driver": "org.postgresql.Driver"}
