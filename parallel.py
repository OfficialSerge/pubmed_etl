            ...
            if "MeshHeadingList" in xml_dict:
                if type(xml_dict["MeshHeadingList"]["MeshHeading"]) == list:
                    for mesh in xml_dict["MeshHeadingList"]["MeshHeading"]:
                        if (
                            "@Type" in mesh["DescriptorName"].keys()
                            and mesh["DescriptorName"]["@Type"] == "Geographic"
                        ):
                            continue
                    ...
                    if not "@Type" in mesh.keys() or mesh["@Type"] != "Geographic":
                        mesh_headings.append(
                            xml_dict["MeshHeadingList"]["MeshHeading"][
                                "DescriptorName"
                            ]["@UI"]
                        )
        ...
        try:
            if "ChemicalList" in xml_dict:
                if type(xml_dict["ChemicalList"]["Chemical"]) == list:
                    ...
                    if (
                        xml_dict["ChemicalList"]["Chemical"]["NameOfSubstance"]["@UI"][
                            0
                        ].lower()
                        == "c"
                    ):
                        chemical_list.append(
                            xml_dict["ChemicalList"]["Chemical"]["NameOfSubstance"][
                                "@UI"
                            ]
                        )
...
def parallelize(file_path, dump_address, mode="parquet"):
    ...
    if mode == "postgres":
        ...
        xml_name = file_path.split(
            ...
            0
        ]  # Example '/shared/hossein_hm31/xml_data/pubmed23n0933.xml' -> pubmed23n0933.xml
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="xml",
        type=str,
        help="Path to XML data directory. Each XML must be in PubMed standard format",
    )
    parser.add_argument(
        "-parquet", dest="parquet", type=str, help="Path to parquet files directory"
    )
    parser.add_argument(
        "-wrap",
        dest="wrap",
        type=str,
        help="Number of cores. 60 - 80 is suggested for whole baseline on otherwise free Valhalla",
    )
    parser.add_argument(
        "-cores",
        dest="cores",
        type=str,
        help="int 0 or 1. It checks whether to make the number of parquet files equal to xml files",
    )
